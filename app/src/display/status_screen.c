...
#if IS_ENABLED(CONFIG_ZMK_WIDGET_ARASAKA)
...
#if IS_ENABLED(CONFIG_ZMK_WIDGET_ARASAKA)
static struct zmk_widget_arasaka arasaka_widget;
#endif
...
#if IS_ENABLED(CONFIG_ZMK_WIDGET_ARASAKA)
    zmk_widget_arasaka_init(&arasaka_widget, screen);
    lv_obj_align(zmk_widget_arasaka_obj(&arasaka_widget), LV_ALIGN_CENTER, 0, 0);
#endif
